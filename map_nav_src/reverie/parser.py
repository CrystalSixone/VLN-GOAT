...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='reverie')
    parser.add_argument('--output_dir', type=str, default='default', help='experiment id')
    parser.add_argument('--seed', type=int, default=0)

    parser.add_argument('--tokenizer', choices=['bert', 'xlm', 'roberta'], default='roberta')
    ...
    parser.add_argument("--scanvp_cands_file", type=str, default="../datasets/R2R/annotations/scanvp_candview_relangles.json")
    ...
    parser.add_argument('--maxDecode', type=int, default=30, help="max input instruction")
    ...
    parser.add_argument('--speaker_layer_num', type=int, default=3)
    parser.add_argument('--speaker_head_num', type=int, default=4)

    parser.add_argument("--subout", dest="sub_out", type=str, default="tanh")  # tanh, max
    parser.add_argument("--accumulateGrad", dest='accumulate_grad', action='store_true', default=False)
    ...
    parser.add_argument("--obj_type",default="attn",type=str)
    parser.add_argument("--speaker_ctx_mask",default=False,action='store_true')
    parser.add_argument("--env_aug",type=str,default="") # env_edit
    parser.add_argument("--use_objids",action="store_true", default=False)
    parser.add_argument("--pred_objids",type=str,default="v3") 
    parser.add_argument("--last_pos", action='store_true', default=False)

    # others
    parser.add_argument("--mode",type=str,required=True) # train/valid/speaker
    ...
    # Features
    parser.add_argument("--use_aug_env",action="store_true",default=False) 
    parser.add_argument("--env_edit",action="store_true",default=False) 
...
def postprocess_args(args):
    ...
    args.speaker_train_vocab = os.path.join(ROOTDIR, 'REVERIE', 'features', 'reverie_speaker_train_vocab.txt')

    # do intervention
    args.img_zdict_file = os.path.join(ROOTDIR, 'R2R', 'features', 'image_z_dict_clip_50.tsv')
    args.img_zdict_size = 50
    args.instr_zdict_file = os.path.join(ROOTDIR, 'REVERIE', 'features', 'reverie_z_instr_dict.tsv')
