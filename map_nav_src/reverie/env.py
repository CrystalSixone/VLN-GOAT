...
from utils.data import angle_feature, get_all_point_angle_feature, calculate_vp_rel_pos_fts, get_angle_fts, get_view_rel_angles
...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        if type(feat_db) is list: 
            ...
            self.feat_db = feat_db[0]
            self.aug_feat_db = feat_db[1]
        else:
    ...
    def getStates(self):
        ...
        for i, sim in enumerate(self.sims):
            ...
            if self.aug_feat_db:
                if i % 2 == 0:
                    # alternatively use aug features and ori features
                    feature = self.aug_feat_db.get_image_feature(state.scanId, state.location.viewpointId)
                else:
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        ...
        if self.tok is not None:
            if 'speaker' in args.mode: # train speaker
                for i in range(len(self.data)):
                    self.data[i]['instr_encoding'] = self.tok.encode_sentence(self.data[i]['instruction'])
        ...
        # For CFP traj feature extraction
        ...
        if scanvp_cands_file is not None:
            try:
                self.scanvp_cands = json.load(open(scanvp_cands_file))
            except Exception:
                self.scanvp_cands = json.load(open(scanvp_cands_file[3:]))
        ...
        self.gt_insts = defaultdict(lambda:[])
        for item in self.data:
            if 'objId' in item and item['objId'] is not None:
                ...
                self.gt_insts[str(item['path_id'])+'_'+str(item['objId'])].append(item['instruction'])
        ...
        # For speaker.
        self.speaker_angle_feat_size = 128
        self.speaker_angle_feature = get_all_point_angle_feature(self.sim,self.speaker_angle_feat_size)
    ...
    def make_candidate(self, feature, scanId, viewpointId, viewId):
        ''' Change: find candidate image and pano objects (v3)
        '''
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    speaker_angle_feat = angle_feature(loc_heading, loc_elevation, self.speaker_angle_feat_size)
                    ...
                    if (loc.viewpointId not in adj_dict or
                        ...
                        adj_dict[loc.viewpointId] = {
                            ...
                            'speaker_feature': np.concatenate((visual_feat, speaker_angle_feat), -1)
                        }
            ...
            for c in candidate:
                ...
                speaker_angle_feat = angle_feature(c_new['heading'], c_new['elevation'], self.speaker_angle_feat_size)
                c_new['speaker_feature'] = np.concatenate((visual_feat, speaker_angle_feat), -1)
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            navigator_feature = np.concatenate((feature, self.angle_feature[base_view_id]), -1)
            speaker_feature = np.concatenate((feature,self.speaker_angle_feature[base_view_id]),-1)

            # objects
            obj_img_fts, obj_ang_fts, obj_box_fts, obj_ids, obj_names = self.obj_db.get_object_feature(
            ...
            ob = {
                ...
                'teacher' : self._teacher_path_action(state, item['path']),
                'obj_name': obj_names
            }
        
            if item['objId'] is not None or len(obj_ids) == 0:
                ob['gt_obj_id'] = item['objId']
            else:
                ob['gt_obj_id'] = np.random.choice(obj_ids)
            if 'instr_encoding' in item:
                if len(item['instr_encoding']) == 1:
                    ob['instr_encoding'] = item['instr_encoding'][0]
                else:
