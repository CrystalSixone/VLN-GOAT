...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if inputs['traj_reverie_loc_fts'] is not None: # REVERIE
            output['traj_reverie_loc_fts'] = [torch.from_numpy(x) for x in inputs['traj_reverie_loc_fts']]
        ...
        if 'instr_z_direction_features' in inputs.keys():
            ...
            output['instr_z_landmark_features'] = inputs['instr_z_landmark_features']
            output['instr_z_landmark_pzs'] = inputs['instr_z_landmark_pzs']
...
def mlm_collate(inputs):
    ...
    if 'instr_z_direction_features' in batch:
        batch['instr_z_direction_features'] = pad_tensors(batch['instr_z_direction_features'])
        batch['instr_z_direction_pzs'] = pad_tensors(batch['instr_z_direction_pzs']).unsqueeze(-1)
        batch['instr_z_landmark_features'] = pad_tensors(batch['instr_z_landmark_features'])
        batch['instr_z_landmark_pzs'] = pad_tensors(batch['instr_z_landmark_pzs']).unsqueeze(-1)

        traj_img_len = batch['traj_view_img_fts'].shape[0]
        batch['img_z_features'] = batch['img_z_features'][0].repeat(traj_img_len,1).reshape(traj_img_len,-1,768)
...
class CfpDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output['extra_heads'] = self.nav_db.args.cfp_extra_head
